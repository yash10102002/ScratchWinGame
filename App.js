...
export default class App extends React.Component {
  ...
  generateRandomNumber = () => {
    ...
    this.setState({ randomNumber, isScratchedd: true })
    return randomNumber
  }

  scratchItem = itemNumber => {
    if(this.state.attemp < 5){
      if(this.state.randomNumber === itemNumber){
        itemArray[itemNumber] = "lucky"
      }else{
        itemArray[itemNumber] = "unlucky" 
      }
      this.setState({attemp: this.state.attemp + 1})
    }else{
      Alert.alert("Max Attemp reached")
    }
    this.forceUpdate()
  }
  ...
  showAllItem = () => {
    ...
    this.setState({attemp:0})
    this.forceUpdate()
  }

  resetGame = () => {
    this.setState({randomNumber: this.generateRandomNumber(), attemp: 0},() => {
